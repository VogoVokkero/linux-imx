...
static ssize_t
isl12057_sysfs_show_osf(struct device *dev,
		       struct device_attribute *attr, char *buf)
{
	struct isl12057_rtc_data *data = dev_get_drvdata(dev->parent);
	unsigned int sr;
	int ret;
	...
	if (ret) {
		...
		int osf = (sr & ISL12057_REG_SR_OSF) ? 1 : 0;
		return sprintf(buf, "%d\n", osf);
	}
}

static ssize_t
isl12057_sysfs_store_osf(struct device *dev,
...
static DEVICE_ATTR(osf, S_IRUGO | S_IWUSR,
		   isl12057_sysfs_show_osf,
		   isl12057_sysfs_store_osf);

static struct attribute *isl12057_rtc_attrs[] = {
	&dev_attr_osf.attr,
	NULL
};

static const struct attribute_group isl12057_rtc_sysfs_files = {
	.attrs	= isl12057_rtc_attrs,
};
			  ...
			  const struct i2c_device_id *id)
{
	...
	ret = rtc_add_group(data->rtc, &isl12057_rtc_sysfs_files);
	if (ret) {
